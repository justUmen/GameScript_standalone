...
10) { echo -n 10 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "In this chapter we will learn how we can create and manage them all."; restore=$(expr $restore + 1) ;&
...
17) { echo -n 17 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "But there are absolutely no difference between these two words. Sometimes you will see one, sometimes the other."; restore=$(expr $restore + 1) ;&
...
20) { echo -n 20 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "This flattened tree body part was called 'paper'."; restore=$(expr $restore + 1) ;&
...
25) { echo -n 25 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "So back to the folder. It was called a 'folder' because this paper was double the size of a normal paper, you could litteraly 'fold' it and you can put several papers inside."; restore=$(expr $restore + 1) ;&
...
30) { echo -n 30 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "But from the side it will look like this, the part going above the rest is used for a label : the name of the folder."; restore=$(expr $restore + 1) ;&
...
35) { echo -n 35 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "But computers dont have the limitations of the real world, because as we all know : the real world sucks and was created for old people."; restore=$(expr $restore + 1) ;&
36) { echo -n 36 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "On a computer, unlike the real world, you can have an unlimited amount of folders inside each other."; restore=$(expr $restore + 1) ;&
...
47) { echo -n 47 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "I'm gonna give you the system that I'm using every day, and I invite you to use it as well... if you don't already have something better."; restore=$(expr $restore + 1) ;&
...
53) { echo -n 53 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "So first, like I said, we need to find a good location."; restore=$(expr $restore + 1) ;&
54) { echo -n 54 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "Let's start our journey on my user's directory. ( /home/justumen/ )"; restore=$(expr $restore + 1) ;&
...
59) { echo -n 59 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "If I'm downloading a big piece of data, a video for example, I don't want it to be in the Sync directory !"; restore=$(expr $restore + 1) ;&
60) { echo -n 60 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "It would be a waste of space and bandwidth, I'll put it in "/home/justumen/Videos" or whatever else."; restore=$(expr $restore + 1) ;&
...
65) { echo -n 65 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "But if the directory is already crowded, like mine is, you may have to organize your scripts by categories."; restore=$(expr $restore + 1) ;&
...
68) { echo -n 68 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "So inside their 'Script' directory, they will have directories like 'Python', 'Perl', 'Bash' and so on..."; restore=$(expr $restore + 1) ;&
...
70) { echo -n 70 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "It's all about taste, there is no right or wrong way to do it.. but people organising them by languages are stupid."; restore=$(expr $restore + 1) ;&
71) { echo -n 71 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "So let's talk first about what i want our script to do, otherwise what I will say won't make any sense."; restore=$(expr $restore + 1) ;&
72) { echo -n 72 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "Let's say, we want our new script to make several queries to all the main search engines and return only the results that they all have in common."; restore=$(expr $restore + 1) ;&
73) { echo -n 73 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "This script will obviously have to crawl the web like a spider... on the web."; restore=$(expr $restore + 1) ;&
...
76) { echo -n 76 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "If a script is in this folder, we know that it will not work properly if the computer is offline."; restore=$(expr $restore + 1) ;&
...
79) { echo -n 79 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "And that's it for the first step, we finaly have selected a good location for our new file !!"; restore=$(expr $restore + 1) ;&
...
83) { echo -n 83 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "Note that on all real Operating Systems, 'example' and 'Example' are two different names, a capital letter is just considered as a totally different letter."; restore=$(expr $restore + 1) ;&
...
85) { echo -n 85 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "So if you read somewhere the sentence "the password is case-sensitive" it means that an uppercase 'A' and lowercase 'a' are considered to be different letters."; restore=$(expr $restore + 1) ;&
...
95) { echo -n 95 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "In many cases, shell included, spaces are already used to separate elements from each other."; restore=$(expr $restore + 1) ;&
...
97) { echo -n 97 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "We will look at this later, but for now just remember : Spaces are not for filenames !"; restore=$(expr $restore + 1) ;&
...
104) { echo -n 104 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "In our example, the element i want to create is a directory. So "SearchEngine" is what we will be using here."; restore=$(expr $restore + 1) ;&
...
109) { echo -n 109 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "The worst name is easy to find, something like 'file', 'this_file' or 'my_script' are indeed all very bad choices."; restore=$(expr $restore + 1) ;&
110) { echo -n 110 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "Files and directories are uniquely identified by the combinaison of their name and their path."; restore=$(expr $restore + 1) ;&
...
112) { echo -n 112 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "So if you respect the rule I've talked about before and use a real OS, you can have a directory called "Example" and a file called "example" in the same place."; restore=$(expr $restore + 1) ;&
...
115) { echo -n 115 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "You should be able to understand what the file contains just by looking at the path of its folder and its name."; restore=$(expr $restore + 1) ;&
...
132) { echo -n 132 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "We already have a good path so now we need a good name."; restore=$(expr $restore + 1) ;&
...
135) { echo -n 135 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen ""common_results" is definitely not a good name by itself !!"; restore=$(expr $restore + 1) ;&
...
141) { echo -n 141 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "Something we won't do here, because it's not what this chapter is about..."; restore=$(expr $restore + 1) ;&
...
147) { echo -n 147 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "A small picture, a normal sized picture and a big picture for let's say : rick, jerry and morty."; restore=$(expr $restore + 1) ;&
...
159) { echo -n 159 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "This "third person" that I was talking about, might be you from the future !!!"; restore=$(expr $restore + 1) ;&
...
162) { echo -n 162 > $HOME/.GameScript/restore_data1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_data1; } 2>/dev/null ; talk justumen "But even if I'm already very impressed of you for just being there, I can't give you any rewards before the quiz..."; restore=$(expr $restore + 1) ;&
esac
}
CLREOL=$'\x1B[K'
