...
restore=$1
case $1 in
1) echo -n 1 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; [ -d "$HOME/.GameScript_bash2" ] && echo "Unexpected error, ${HOME}/.GameScript_bash3 already exists on your system! Delete this $HOME/.GameScript_bash3 folder and restart this script." && exit; restore=$(expr $restore + 1) ;&
...
45) echo -n 45 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Here you have not only created the text file 'test', but you also gave it a content."; restore=$(expr $restore + 1) ;&
...
48) echo -n 48 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Again, just give ${code}cat${reset} the argument to the name of the file, just as you would for ${code}rm${reset}."; restore=$(expr $restore + 1) ;&
...
51) echo -n 51 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "The command ${code}cat${reset} has this name because it can be used to make a con${code}cat${reset}enation, which is basically putting chains of character end to end."; restore=$(expr $restore + 1) ;&
...
64) echo -n 64 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Be mindful of ${code}>${reset}, because it can delete the contents of your files."; restore=$(expr $restore + 1) ;&
...
122) echo -n 122 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "In ${code} \\\\\\exit ${reset}, the first ${code}\${reset} is the escape character, but the second one is just the character that must be interpreted literally by your terminal."; restore=$(expr $restore + 1) ;&
...
124) echo -n 124 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Two ${code}\"${reset} can act as a ${code}delimiter${reset} of arguments."; restore=$(expr $restore + 1) ;&
...
146) echo -n 146 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Now instead of displaying the result in the terminal, put it in a file with named 'tab'."; restore=$(expr $restore + 1) ;&
...
154) echo -n 154 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Excellent! You are ready for the quiz."; restore=$(expr $restore + 1) ;&
