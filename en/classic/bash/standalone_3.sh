...
37) { echo -n 37 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk justumen "When you use the ${code}echo${reset} command, the result will be displayed on your device."; restore=$(expr $restore + 1) ;&
...
46) { echo -n 46 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk justumen "This content will be the result of the command to the left of the ${code}>${reset} symbol."; restore=$(expr $restore + 1) ;&
...
56) { echo -n 56 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk justumen "${reset} cat test test${reset} simply displays the contents of the 'test' file twice."; restore=$(expr $restore + 1) ;&
...
87) { echo -n 87 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; answer_run "pwd >>" justumen "No test"; restore=$(expr $restore + 1) ;&
...
104) { echo -n 104 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk_not_press_key justumen "Try with this escape character: ${learn}echo a \\ b${reset}"; restore=$(expr $restore + 1) ;&
...
116) { echo -n 116 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk justumen "Here, you can understand the ${} Voice huge reset} ${difference there is between ${learn}echo \ x> y {reset} $ and ${learn}echo x> y${reset} ."; restore=$(expr $restore + 1) ;&
...
120) { echo -n 120 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk_not_press_key justumen "For example, try to display in your terminal: ${code}\\exit${reset}."; restore=$(expr $restore + 1) ;&
121) { echo -n 121 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; answer_run "echo \\\\exit" justumen "No"; restore=$(expr $restore + 1) ;&
122) { echo -n 122 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk justumen "In ${code} \\\\\\exit ${reset}, the first ${code}\${reset} is the escape character, but the second one is just the character that must be interpreted literally by your terminal."; restore=$(expr $restore + 1) ;&
...
141) { echo -n 141 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; answer_run "echo -e 'a \ nb'" justumen "No"; restore=$(expr $restore + 1) ;&
142) { echo -n 142 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; talk_not_press_key justumen "Find the syntax of a ${var}${reset} tab ${} tab in the ${code}echo${reset} command manual."; restore=$(expr $restore + 1) ;&
...
145) { echo -n 145 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; answer_run "echo -e 'a \ tb'" justumen "No"; restore=$(expr $restore + 1) ;&
...
147) { echo -n 147 > $HOME/.GameScript/restore_bash3; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; } 2>/dev/null ; answer_run "echo -e 'a \ tb'> tab" justumen "No"; restore=$(expr $restore + 1) ;&
