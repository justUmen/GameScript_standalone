...
49) { echo -n 49 > $HOME/.GameScript/restore_bash4; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; } 2>/dev/null ; talk_not_press_key justumen "Move 'test2' to the 'DOS2' folder and rename it to 'test3' with : ${learn}mv test2 DOS2/1test3${reset}"; restore=$(expr $restore + 1) ;&
...
53) { echo -n 53 > $HOME/.GameScript/restore_bash4; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; } 2>/dev/null ; talk justumen "The file 'test2' has been moved to DOS2 ${voc}and${reset} renamed to 'test3'."; restore=$(expr $restore + 1) ;&
...
98) { echo -n 98 > $HOME/.GameScript/restore_bash4; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; } 2>/dev/null ; talk_not_press_key justumen "Now, try to move to the DOS folder and create a new folder inside called 'NEW'."; restore=$(expr $restore + 1) ;&
...
119) { echo -n 119 > $HOME/.GameScript/restore_bash4; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; } 2>/dev/null ; talk_not_press_key justumen "Try with a typo in the command: ${learn}lss||echo test${reset}"; restore=$(expr $restore + 1) ;&
...
127) { echo -n 127 > $HOME/.GameScript/restore_bash4; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; } 2>/dev/null ; talk_not_press_key justumen "Here the folder 'DOS' does not exist, so use DOS2 instead with this command: ${learn}cd DOS2&&ls||pwd${reset}"; restore=$(expr $restore + 1) ;&
