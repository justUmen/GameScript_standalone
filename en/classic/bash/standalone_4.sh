...
restore=$1
case $1 in
1) echo -n 1 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; [ -d "$HOME/.GameScript_bash4" ] && echo "Unexpected error, ${HOME}/.GameScript_bash4 already exists on your system! Delete this $HOME/.GameScript_bash4 folder and restart this script." && exit; restore=$(expr $restore + 1) ;&
...
4) echo -n 4 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "In the previous chapter, we saw together how to create and edit text files."; restore=$(expr $restore + 1) ;&
...
12) echo -n 12 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "To move a file you will have to use the command ${code}mv${reset}."; restore=$(expr $restore + 1) ;&
13) echo -n 13 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "As a reminder : 'm' and 'v' are the consonants of the word 'move'."; restore=$(expr $restore + 1) ;&
...
15) echo -n 15 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; answer_run "mv test DIR" justumen "No"; restore=$(expr $restore + 1) ;&
...
18) echo -n 18 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk_not_press_key justumen "Move your terminal inside the 'DIR' folder."; restore=$(expr $restore + 1) ;&
...
24) echo -n 24 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "You moved the file 'test' in the directory 'DIR' with this command: ${learn}mv test DIR${reset}"; restore=$(expr $restore + 1) ;&
...
28) echo -n 28 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "But you should be careful, because ${code}mv${reset} can have either files or folders as arguments."; restore=$(expr $restore + 1) ;&
29) echo -n 29 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "If the first argument is a file and the second is not a folder, the command will act differently : this time the first argument will be ${voc}renamed${reset}."; restore=$(expr $restore + 1) ;&
30) echo -n 30 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "Be careful to understand your environment, you should know what are the files and folders in your current location."; restore=$(expr $restore + 1) ;&
31) echo -n 31 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "In your current directory you still have this file 'test'."; restore=$(expr $restore + 1) ;&
...
41) echo -n 41 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "I repeat, ${code}mv${reset} can have files or folders as arguments."; restore=$(expr $restore + 1) ;&
42) echo -n 42 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "If the first argument is a folder, and the second one does not exist, then ${code}mv${reset} will just rename the folder given as the first argument."; restore=$(expr $restore + 1) ;&
43) echo -n 43 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk_not_press_key justumen "Rename the folder 'DOS' in 'DOS2'."; restore=$(expr $restore + 1) ;&
...
47) echo -n 47 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "So be careful with the command ${code}mv${reset}, it can have two different roles : move and rename."; restore=$(expr $restore + 1) ;&
...
54) echo -n 54 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk_not_press_key justumen "Display the content of this 'test3' file."; restore=$(expr $restore + 1) ;&
...
56) echo -n 56 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "The content is obviously still the same."; restore=$(expr $restore + 1) ;&
57) echo -n 57 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "If you try add the text 'everyone' after this file like we did before, this word will be added on a separate line."; restore=$(expr $restore + 1) ;&
58) echo -n 58 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "Because when you use ${code}echo${reset}, it will automaticaly add a new empty line at the end."; restore=$(expr $restore + 1) ;&
59) echo -n 59 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "If you do not want this new empty line, you will have to use the ${learn}-n${reset} option of the ${code}echo${reset} command."; restore=$(expr $restore + 1) ;&
60) echo -n 60 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk_not_press_key justumen "Replace the contents of the file 'test3' with 'hello', but without this new empty line."; restore=$(expr $restore + 1) ;&
...
66) echo -n 66 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "And here it is : hello everyone!"; restore=$(expr $restore + 1) ;&
67) echo -n 67 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "When a command doesn't do exactly what you want it to do, visit its manual !"; restore=$(expr $restore + 1) ;&
...
72) echo -n 72 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "When a commands has two arguments, most of the time this logic (${code}<COMMAND> <SOURCE> <DESTINATION>${reset}) applies."; restore=$(expr $restore + 1) ;&
73) echo -n 73 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "The first argument of the command is the ${voc}source${reset}, the second is the ${voc}destination${reset}."; restore=$(expr $restore + 1) ;&
74) echo -n 74 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "To ${voc}copy${reset} or ${voc}duplicate${reset} a file, you will have to use the command ${code}cp${reset}."; restore=$(expr $restore + 1) ;&
...
88) echo -n 88 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "This result shouldn't be strange for you."; restore=$(expr $restore + 1) ;&
...
90) echo -n 90 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "${code}.new${reset} is obviously a hidden file, because it starts with a dot."; restore=$(expr $restore + 1) ;&
91) echo -n 91 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk_not_press_key justumen "List the hidden files in the current directory to find it."; restore=$(expr $restore + 1) ;&
...
95) echo -n 95 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "It is also possible to have several commands on a single line, you just have to separate them by ${code};${reset}."; restore=$(expr $restore + 1) ;&
...
102) echo -n 102 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "With ${code};${reset}, whatever the result of the first command is, the second one will be launched."; restore=$(expr $restore + 1) ;&
...
105) echo -n 105 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "But you can also create conditions before moving on to the next command."; restore=$(expr $restore + 1) ;&
...
113) echo -n 113 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "Because the command ${learn}cd DOS${reset} returned an error, ${code}&&${reset} has blocked the execution of the next command."; restore=$(expr $restore + 1) ;&
...
129) echo -n 129 > $HOME/.GameScript/restore_bash4; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash4; talk justumen "Because the 'DOS2' folder exists, the ${learn}pwd${reset} command will not be executed."; restore=$(expr $restore + 1) ;&
