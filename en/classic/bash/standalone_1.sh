...
4) { echo -n 4 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Hello everyone and welcome to the first chapter about 'bash'."; restore=$(expr $restore + 1) ;&
...
16) { echo -n 16 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "${code}/home/user/${reset} represent the directory 'user', which is in the directory 'home', which is itself in the ${voc}root directory${reset}."; restore=$(expr $restore + 1) ;&
...
25) { echo -n 25 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "${codeError}file1${reset} can't exist because there is already a file with the same name in the same directory. Here the root directory : ${learn}/file1${reset}"; restore=$(expr $restore + 1) ;&
26) { echo -n 26 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "But above it, the other file ${codeFile}/home/file1${reset} can exist, because even if their name is the same, they are not directly in the same folder."; restore=$(expr $restore + 1) ;&
...
28) { echo -n 28 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "The file ${codeError}/home${reset} can't exist either, because there is already a folder ${code}/home/${reset}, that is using the same name in the same place."; restore=$(expr $restore + 1) ;&
...
32) { echo -n 32 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "For ${codeError}home${reset}, we need to give another name that won't be a problem : like for example 'Home'."; restore=$(expr $restore + 1) ;&
33) { echo -n 33 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Yes ! On Unix, uppercase letters are important. 'Home' and 'home' are two different names."; restore=$(expr $restore + 1) ;&
...
37) { echo -n 37 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "It is also possible to represent the same folders this way."; restore=$(expr $restore + 1) ;&
...
39) { echo -n 39 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "And here is the same example with files. Identical to the tree we saw earlier."; restore=$(expr $restore + 1) ;&
...
44) { echo -n 44 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "This kind of line, which start with the ${voc}root directory${reset} '/', is called the ${voc}absolute path${reset} of a folder or file."; restore=$(expr $restore + 1) ;&
...
49) { echo -n 49 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Let's start by creating a new folder with the command ${learn}mkdir${reset} (This command's name comes from the sentence : ${learn}m${reset}a${learn}k${reset}e ${learn}dir${reset}ectory)."; restore=$(expr $restore + 1) ;&
...
53) { echo -n 53 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk_not_press_key justumen "Let's now display the folders and files, with a simple ${learn}ls${reset} (ls comes from the word : ${learn}l${reset}i${learn}s${reset}t)."; restore=$(expr $restore + 1) ;&
...
57) { echo -n 57 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk_not_press_key justumen "To do that, we just have to do ${code}cd${reset}, followed by the name of the folder, in our case : ${learn}cd House${reset}."; restore=$(expr $restore + 1) ;&
...
61) { echo -n 61 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Here the directory 'House' is empty, it's normal, because you've just created it."; restore=$(expr $restore + 1) ;&
...
68) { echo -n 68 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "This folder, where you are right now, has a special name : it's your ${voc}working directory${reset}, also called ${voc}current directory${reset}."; restore=$(expr $restore + 1) ;&
...
75) { echo -n 75 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk_not_press_key justumen "Now let's create a new folder, called 'Room', in our ${voc}current directory${reset}, with : ${learn}mkdir Room${reset}"; restore=$(expr $restore + 1) ;&
...
77) { echo -n 77 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk_not_press_key justumen "Change your ${voc}current directory${reset} with : ${learn}cd Room${reset}"; restore=$(expr $restore + 1) ;&
...
83) { echo -n 83 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Here the folder is empty, but you already understand two important commands : ${learn}pwd${reset} and ${learn}ls${reset}."; restore=$(expr $restore + 1) ;&
...
88) { echo -n 88 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "You just have to separate them with spaces."; restore=$(expr $restore + 1) ;&
...
97) { echo -n 97 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "${learn}rm${reset} is the command to delete files. (rm comes from the word ${learn}r${reset}e${learn}m${reset}ove)"; restore=$(expr $restore + 1) ;&
...
99) { echo -n 99 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Just like ${learn}mkdir${reset}, you need to give it an ${voc}argument${reset} : the name of the file you want to target, for example : ${learn}rm test${reset}."; restore=$(expr $restore + 1) ;&
...
107) { echo -n 107 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "But be careful with ${learn}rm${reset}, it is a very dangerous command, and you shouldn't take it lightly !"; restore=$(expr $restore + 1) ;&
...
110) { echo -n 110 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "A mispell or a wrong ${voc}current directory${reset} can have severe consequences."; restore=$(expr $restore + 1) ;&
...
133) { echo -n 133 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Without ${voc}argument${reset}, with a simple ${learn}ls${reset}, the targetted folder will be the ${voc}current directory${reset}."; restore=$(expr $restore + 1) ;&
...
135) { echo -n 135 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "This ${voc}argument${reset} is the targeted folder, for example ${learn}ls /${reset}, will display the content of the ${voc}root directory${reset}."; restore=$(expr $restore + 1) ;&
...
145) { echo -n 145 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Let's imagine two 'virus' files : with their ${voc}absolute path${reset} being : ${learn}/virus${reset} and ${learn}/bin/virus${reset}."; restore=$(expr $restore + 1) ;&
...
148) { echo -n 148 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "This is why this ${voc}absolute path${reset} is very important. You can do ${learn}rm /virus${reset}, or ${learn}rm /bin/virus${reset}, and it will work whatever your ${voc}current directory${reset} is."; restore=$(expr $restore + 1) ;&
...
159) { echo -n 159 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk_not_press_key justumen "Here you can still see some 'virus' files. Delete the file 'virus2' using its ${voc}relative path${reset}."; restore=$(expr $restore + 1) ;&
...
166) { echo -n 166 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "There is also a very useful syntax you can use with the ${voc}relative path${reset} : ${learn}..${reset}"; restore=$(expr $restore + 1) ;&
167) { echo -n 167 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "${learn}..${reset} represents the parent folder of your ${voc}current directory${reset}."; restore=$(expr $restore + 1) ;&
...
172) { echo -n 172 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "Like for example with the command we used before : ${learn}cd House/Room/${reset}"; restore=$(expr $restore + 1) ;&
