...
26) { echo -n 26 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "But above it, the other file named ${codeFile}file1${reset} can exist because even if their name is the same, they are not directly in the same folder."; restore=$(expr $restore + 1) ;&
...
37) { echo -n 37 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "It is also possible to represent the folders we already saw together this way."; restore=$(expr $restore + 1) ;&
...
39) { echo -n 39 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "And here is the same example with files. Identical to the tree we saw."; restore=$(expr $restore + 1) ;&
...
93) { echo -n 93 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk_not_press_key justumen "Now to delete these folders, you can do : ${learn}rmdir bed closet desk${reset}. (rmdir comes from ${learn}e${reset}e${learn}m${reset}ove ${learn}dir${reset}ectory)"; restore=$(expr $restore + 1) ;&
...
97) { echo -n 97 > $HOME/.GameScript/restore_bash1; } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; } 2>/dev/null ; talk justumen "${learn}rm${reset} is the command to delete files. (rm comes from the word ${learn}e${reset}e${learn}m${reset}ove)"; restore=$(expr $restore + 1) ;&
