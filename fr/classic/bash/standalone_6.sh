...
restore=$1
case $1 in
1) echo -n 1 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; [ -d "$HOME/.GameScript_bash6" ] && echo "Erreur inattendue, ${HOME}/.GameScript_bash6 existe déjà sur votre système ! Supprimez ce dossier $HOME/.GameScript_bash6 et relancez ce script." && exit; restore=$(expr $restore + 1) ;&
...
7) echo -n 7 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Nous avons ici quelques problèmes de permission à régler..."; restore=$(expr $restore + 1) ;&
...
66) echo -n 66 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk_not_press_key justumen "Maintenant que ces deux nouveaux symboles sont acquis, affichez le contenu du fichier 'f'."; restore=$(expr $restore + 1) ;&
...
74) echo -n 74 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Prenons cet exemple : -rw------- 1 umen team 10 Feb 20 16:16 f"; restore=$(expr $restore + 1) ;&
...
76) echo -n 76 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "${codeFile}-rw-------${reset} 1 umen team 10 Feb 20 16:16 f : Vous connaissez déjà la première colonne, il s'agit du type de l'élément et de ses permissions."; restore=$(expr $restore + 1) ;&
...
81) echo -n 81 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "-rw------- 1 umen team ${codeFile}10${reset} Feb 20 16:16 f : La cinquième colonne est la ${voc}taille${reset} en ${voc}octet${reset} du fichier. (octect = byte en anglais)"; restore=$(expr $restore + 1) ;&
...
84) echo -n 84 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Pour information, le contenu de notre fichier 'f' est égal en binaire à ça : 01100011 01101111 01101110 01110100 01100101 01101110 01110101 00100000 01100110 00001010"; restore=$(expr $restore + 1) ;&
...
86) echo -n 86 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "-rw------- 1 umen team 10 Feb 20 16:16 ${codeFile}f${reset} : Et la dernière colonne est simplement le nom du fichier."; restore=$(expr $restore + 1) ;&
...
109) echo -n 109 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Il faudra ensuite faire l'addition des permissions, 'rw-' sera donc 4+2=${voc}6${reset}, 'r-x' sera 4+1=${voc}5${reset}, 'r--' sera ${voc}4${reset}, 'rwx' sera 4+2+1=${voc}7${reset}, '---' sera bien évidemment ${voc}0${reset}, etc..."; restore=$(expr $restore + 1) ;&
...
111) echo -n 111 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Par exemple, sur le fichier 'test', pour donner tous les droits au propriétaire mais aucun au reste, il faudra faire ${learn}chmod 700 test${reset}"; restore=$(expr $restore + 1) ;&
112) echo -n 112 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "${codeFile}7${reset}00 donne toutes les permissions au propriétaire : ${codeFile}rwx${reset}"; restore=$(expr $restore + 1) ;&
...
122) echo -n 122 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Il faudra d'abord réinitialiser toutes les permissions à 0 avec ${learn}chmod ugo-rwx ??${reset}."; restore=$(expr $restore + 1) ;&
123) echo -n 123 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Ensuite ${learn}chmod ugo+r ??${reset} pour faire l'équivalent de ${learn}chmod 444 ??${reset}."; restore=$(expr $restore + 1) ;&
124) echo -n 124 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Et il faudra ensuite transformer ${code}444${reset} en ${code}644${reset} avec ${learn}chmod u+w ??${reset}."; restore=$(expr $restore + 1) ;&
125) echo -n 125 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Cependant l'inverse n'est pas du tout possible, ${code}u+w${reset} n'a pas d'équivalent en chiffre, parce que le résultat dépendra des permissions précédentes."; restore=$(expr $restore + 1) ;&
126) echo -n 126 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Les deux méthodes ne sont pas pas équivalentes !"; restore=$(expr $restore + 1) ;&
127) echo -n 127 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Ca sera donc à vous de choisir quelle méthode vous souhaitez utiliser, en fonction de votre situation."; restore=$(expr $restore + 1) ;&
128) echo -n 128 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Et pour en terminer avec les bases sur les permissions, nous allons voir comment changer de propriétaire et de groupe."; restore=$(expr $restore + 1) ;&
129) echo -n 129 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "La commande à utiliser sera : ${code}chown${reset}, abréviation anglaise de : ${code}ch${reset}ange ${code}own${reset}er."; restore=$(expr $restore + 1) ;&
130) echo -n 130 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk_not_press_key justumen "Pour mettre ${voc}root${reset} comme propriétaire du fichier 'f' vous pouvez simplement taper : ${learn}chown root f${reset}"; restore=$(expr $restore + 1) ;&
131) echo -n 131 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; answer_run "chown root f" justumen "Non"; restore=$(expr $restore + 1) ;&
132) echo -n 132 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Ici cette commande ne fonctionne pas ! C'est encore une fois un problème de permission, mais qui n'est pas du même type."; restore=$(expr $restore + 1) ;&
...
135) echo -n 135 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Mais je ne vous demanderai pas votre mot de passe ${voc}root${reset} dans GameScript."; restore=$(expr $restore + 1) ;&
136) echo -n 136 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Je vous invite donc à tester cette commande ${code}chown${reset} plus tard, en root, par vous même dans un autre terminal !"; restore=$(expr $restore + 1) ;&
137) echo -n 137 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "Pour vous connectez en ${code}root${reset} vous pouvez faire : ${learn}su root${reset}. Votre mot de passe vous sera alors demandé et vous aurez votre terminal en ${code}root${reset}."; restore=$(expr $restore + 1) ;&
...
142) echo -n 142 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "${voc}root${reset} possède TOUTES les permissions pour TOUS les éléments !"; restore=$(expr $restore + 1) ;&
143) echo -n 143 > $HOME/.GameScript/restore_bash6; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash6; talk justumen "${voc}root${reset} est non seulement capable de supprimer tous vos fichiers, mais il a aussi le pouvoir de rendre votre système inutilisable !"; restore=$(expr $restore + 1) ;&
