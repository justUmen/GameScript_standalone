...
talk justumen "Ici nous voulons rajouter le droit de lecture et d'écriture pour le propriétaire sur tous les éléments du répertoire courant."
...
talk justumen "Utiliser cette méthode est finalement particulièrement rébarbative, surtout si vous avez un grand nombre de fichiers à changer."
...
talk justumen "Mais ${code}*${reset} peut aussi être utilisé avec lessautres commandes."
talk_not_press_key justumen "Affichez tous les éléments du répertoire courant tous les uns à la suite des autres."
...
talk_not_press_key justumen "Par example, vous pouvez afficher tous les éléments du répertoire courant qui commence par un 'f' avec : ${learn}ls f*${reset}"
answer_run "ls f*" justumen "Non"
talk justumen "Ici le fichier 'intrus' n'est pas présent car son nom ne commence pas par un 'f'."
talk_not_press_key justumen "Ou encore afficher tous les éléments du répertoire courant qui se terminent par un 0 avec : ${learn}ls *0${reset}"
...
talk_not_press_key justumen "Lister maintenant tous les éléments du répertoire courant qui commence par un 'i' et finissent par un 's'."
...
talk justumen "Si vous avez besoin d'un ciblage plus en finesse, vous puovez utilise un autre ${voc}joker${reset} : le ${code}?${reset}."
...
talk_not_press_key justumen "Vous pouvez donc utiliser ${code}?${reset} pour afficher les permissions de tous les fichiers avec un nom d'une lettre : ${learn}ls -l ?${reset}"
answer_run "ls -l ?" justumen "Non"
...
talk justumen "Une commande comme ${learn}rm *${reset} peut avoir des conséquences graves."
...
talk_not_press_key justumen "Affichez maintenant les résultats de ${learn}ls -l${reset}."
answer_run "ls -l" justumen "Non"
...
talk justumen "Il y a en fait 7 colonnes."
...
talk justumen "-rw------- 1 umen ${codeFile}team${code} 10 Feb 20 16:16 f : La quatrième colonne est je le rappelle le nom du groupe."
...
talk justumen "Ici le contenu de notre fichier 'f' est égal en binaire à 01100011 01101111 01101110 01110100 01100101 01101110 01110101 00100000 01100110 00001010"
talk justumen "-rw------- 1 umen team 10 ${code}FileFeb 20 16:16${reset} f : La sixième colonne est la date de la dernière modification du fichier."
...
talk_not_press_key justumen "Affichez enfin les métadonnées du fichier 'f'."
answer_run "ls -l f" justumen "Non"
...
talk justumen "Et évidemment, cat ne fait qu'afficher le fichier sans le moddifier, donc la date ne change pas."
...
talk justumen "Mais il est aussi possible d'utiliser chmod avec 3 chiffres !"
...
talk justumen "${codeFile}7${reset}00 donne toutes les permission au propriétaire : ${codeFile}rwx${reset}"
...
talk justumen "Pour vous donner un autre exemple, donner tous les droits à tout le monde sur le fichier 'test' : ${learn}chmod 777 test${reset}"
...
talk justumen "Les deux méthode ne sont pas équivalentes."
...
talk_not_press_key justumen "Pour mettre root propriétaire du fichier 'f' vous pouvez simplement taper : ${learn}chown root f${reset}"
...
talk justumen "Par mesure de sécurité, certaines commandes peuvent uniquement être utilisé par l'utilisateur administrateur : root."
talk justumen "${code}chown${reset} est l'une d'entre elles, c'est un commande que seul ${voc}root${reset} peut utiliser !"
talk justumen "Mais je ne vous demanderai pas votre mot de passe ${voc}root${reset} dans ce script."
talk justumen "Je vous invite donc à tester cette commande en root par vous même dans un autre terminal plus tard !"
talk justumen "Pour vous connectez en root vous pouvez faire : ${learn}su root${reset}, votre mot de passe vous sera alors demandé et vous aurez votre terminal en root."
talk justumen "Pour tester si vous êtes bien ${voc}root${reset} vous pouvez utiliser la commande : ${learn}whoami${reset}."
talk justumen "Si vous voulez changer le groupe il faudra utiliser les ${code}:${code}, comme par exemple ${learn}chown :familleEinstein f${reset}."
...
talk justumen "Attention cependant à toutes les commandes que vous lancer en ${voc}root${reset} !"
talk justumen "${voc}root${reset} possède TOUTES les permissions pour TOUT !"
...
talk justumen "Une commande maladroite en ${voc}root${reset} peut avoir des effets devastateurs."
talk justumen "C'est tout pour cette fois. Vous maitrisez maintenant tout ce qu'il vous faut pour maitriser vos permissions."
