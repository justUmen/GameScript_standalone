...
172) echo -n 172 > $HOME/.GameScript/restore_bash1 2>/dev/null ; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1 2>/dev/null ; talk justumen "Comme par exemple avec la commande de tout a l'heure : ${learn}cd House/Room/${reet}"; restore=$(expr $restore + 1) ;&
