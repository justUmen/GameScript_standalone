...
2) echo -n 2 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; mkdir $HOME/.GameScript_bash1; restore=$(expr $restore + 1) ;&
3) echo -n 3 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; cd $HOME/; restore=$(expr $restore + 1) ;&
4) echo -n 4 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Hey salut tout le monde et bienvenu sur la ligne de commande linux."; restore=$(expr $restore + 1) ;&
5) echo -n 5 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Avant de pouvoir apprendre notre première commande, il va falloir d'abord comprendre la logique derrière l'organisation des répertoires et des fichiers sur Linux."; restore=$(expr $restore + 1) ;&
...
8) echo -n 8 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Vous pouvez imaginer le système d'organisation des dossiers linux comme un arbre."; restore=$(expr $restore + 1) ;&
9) echo -n 9 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Dans cette arbre les lignes qui représentent les dossiers sont en bleu ciel."; restore=$(expr $restore + 1) ;&
...
27) echo -n 27 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Les éléments d'un système linux doivent avoir une référence unique : ici ${learn}/fichier1${reset} et ${learn}/home/fichier1${reset} ne sont pas en conflit."; restore=$(expr $restore + 1) ;&
...
32) echo -n 32 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Pour ${codeError}home${reset}, il suffit également de lui donner un autre nom qui ne pose pas de problème comme 'Home'."; restore=$(expr $restore + 1) ;&
33) echo -n 33 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Oui ! Sur linux, les majuscules sont importantes. 'Home' et 'home' sont deux noms différents."; restore=$(expr $restore + 1) ;&
34) echo -n 34 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Quand les majuscules ne sont pas identiques au minuscules, on dit que les noms sont ${voc}sensibles à la casse${reset}."; restore=$(expr $restore + 1) ;&
35) echo -n 35 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Effectivement, linux est ${voc}sensible à la casse${reset}. C'est à dire que 'home', 'Home', 'hOme', 'hoMe', 'homE', 'HoMe', 'hOmE', 'HOme', 'hoME', 'HomE', 'hOMe', 'HOME', etc... sont tous des noms valides et différents !"; restore=$(expr $restore + 1) ;&
...
39) echo -n 39 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Et ici le même exemple avec les fichiers. Identique à l'arbre du dessus."; restore=$(expr $restore + 1) ;&
...
44) echo -n 44 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Ce genre de ligne, qui commence par le ${voc}répertoire racine${reset} '/' s'appelle le ${voc}chemin absolu${reset} d'un fichier ou d'un dossier."; restore=$(expr $restore + 1) ;&
...
47) echo -n 47 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Ce ${voc}chemin absolu${reset} est le concept le plus fondamental de la ligne de commande linux."; restore=$(expr $restore + 1) ;&
48) echo -n 48 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Maintenant nous pouvons voir notre première commande linux."; restore=$(expr $restore + 1) ;&
49) echo -n 49 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Commençons par créer un nouveau dossier avec la commande ${learn}mkdir${reset} (${learn}M${reset}a${learn}K${reset}e ${learn}DIR${reset}ectory)."; restore=$(expr $restore + 1) ;&
...
53) echo -n 53 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Affichons maintenant les fichiers et dossiers avec un simple ${learn}ls${reset} (${learn}L${reset}i${learn}S${reset}t)."; restore=$(expr $restore + 1) ;&
...
56) echo -n 56 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Maintenant rentrons dans ce dossier avec la commande ${learn}cd${reset} (${learn}C${reset}hange ${learn}D${reset}irectory)."; restore=$(expr $restore + 1) ;&
57) echo -n 57 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Pour cela, il suffit de faire cd, suivi du nom du dossier voulu, dans notre cas : ${learn}cd House${reset}."; restore=$(expr $restore + 1) ;&
...
63) echo -n 63 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "En fait, un terminal linux tourne toujours dans un dossier, et peut se 'déplacer' dans l'arborescense du système."; restore=$(expr $restore + 1) ;&
...
65) echo -n 65 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Pour savoir dans quel répertoire votre terminal est en ce moment, il suffit de taper ${learn}pwd${reset} (${learn}P${reset}rint ${learn}W${reset}orking ${learn}D${reset}irectory)."; restore=$(expr $restore + 1) ;&
...
68) echo -n 68 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Ce répertoire où vous êtes porte un nom spécial : le ${voc}${voc}répertoire courant${reset} ${reset}."; restore=$(expr $restore + 1) ;&
69) echo -n 69 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Comme je vous l'ai déjà dit, il n'est pas obligatoire de mettre un / pour le dernier dossier c'est pourquoi vous voyez ici ${learn}$(pwd)${reset} et non pas ${learn}$(pwd)/${reset}."; restore=$(expr $restore + 1) ;&
70) echo -n 70 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Voilà donc 4 commandes linux fondamentales : ${learn}pwd${reset}, ${learn}ls${reset}, ${learn}cd${reset} et ${learn}mkdir${reset}."; restore=$(expr $restore + 1) ;&
...
77) echo -n 77 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Changeons le ${voc}répertoire courant${reset} avec ${learn}cd Room${reset}."; restore=$(expr $restore + 1) ;&
...
83) echo -n 83 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Ici le dossier est vide, mais vous maitrisez maintenant les deux commandes linux les plus importantes : ${learn}pwd${reset} et ${learn}ls${reset}."; restore=$(expr $restore + 1) ;&
...
93) echo -n 93 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Maintenant pour supprimer ces dossiers, vous pouvez taper : ${learn}rmdir bed closet desk${reset}. (${learn}R${reset}e${learn}M${reset}ove ${learn}DIR${reset}ectory)"; restore=$(expr $restore + 1) ;&
...
96) echo -n 96 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Ce qui peut empêcher de grave accidents. Si par exemple, vous faites par erreur ${learn}rmdir /home${reset}."; restore=$(expr $restore + 1) ;&
97) echo -n 97 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "La commande ${learn}rm${reset} est la commande pour supprimer des fichiers. (${learn}R${reset}e${learn}M${reset}ove)"; restore=$(expr $restore + 1) ;&
...
99) echo -n 99 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Tout comme ${learn}mkdir${reset}, il faudra lui donner un ${voc}argument${reset} le nom du fichier en question, par exemple : ${learn}rm test${reset}."; restore=$(expr $restore + 1) ;&
...
107) echo -n 107 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Mais attention avec la commande ${learn}rm${reset}, c'est une commande de professionnel à ne pas utiliser à la légère."; restore=$(expr $restore + 1) ;&
...
112) echo -n 112 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "N'hésitez jamais a lancer ${learn}pwd${reset} et ${learn}ls${reset} pour savoir quel est votre ${voc}répertoire courant${reset} et vérifier son contenu."; restore=$(expr $restore + 1) ;&
113) echo -n 113 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Mais nous avons d'autres virus à supprimer. Mais on peut aussi les supprimer d'une autre manière."; restore=$(expr $restore + 1) ;&
114) echo -n 114 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "En utilisant son ${voc}chemin absolu${reset} dont j'ai déjà parlé auparavant."; restore=$(expr $restore + 1) ;&
115) echo -n 115 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; cd ~/; restore=$(expr $restore + 1) ;&
116) echo -n 116 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Lorsque vous avez tapé ${learn}rm virus${reset}, vous demandez de supprimer le fichier 'virus' dans votre ${voc}répertoire courant${reset}."; restore=$(expr $restore + 1) ;&
...
122) echo -n 122 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Heureusement, vous connaissez le ${voc}chemin absolu${reset} du fichier 'virus1' : ${learn}$(echo $HOME)/House/Room/virus1${Reset}"; restore=$(expr $restore + 1) ;&
123) echo -n 123 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Vous pouvez utiliser son ${voc}chemin absolu${reset} comme ${voc}argument${reset}. Cette commande marchera donc quel que soit votre ${voc}répertoire courant${reset} actuel !"; restore=$(expr $restore + 1) ;&
124) echo -n 124 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Supprimez le avec ${learn}rm $(echo $HOME)/House/Room/virus1${reset}."; restore=$(expr $restore + 1) ;&
125) echo -n 125 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; answer_run "rm $(echo $HOME)/House/Room/virus1" justumen "Non"; restore=$(expr $restore + 1) ;&
126) echo -n 126 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Maintenant, comment pouvoir vérifier que le fichier a bien ete supprimé ?"; restore=$(expr $restore + 1) ;&
127) echo -n 127 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Lorsqu'une commande linux ne se passe pas comme prévue, elle vous renvoit très souvent une erreur."; restore=$(expr $restore + 1) ;&
...
129) echo -n 129 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; answer_run "rm $(echo $HOME)/House/Room/virus1" justumen "Non"; restore=$(expr $restore + 1) ;&
...
134) echo -n 134 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Mais il est en fait aussi possible de donner un ${voc}argument${reset} a ls."; restore=$(expr $restore + 1) ;&
...
136) echo -n 136 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Maintenant nous pouvons afficher le contenu du répertoire 'Room' sans se déplacer dans l'arborescence, avec : ${learn}ls $(echo $HOME)/House/Room/${reset}."; restore=$(expr $restore + 1) ;&
137) echo -n 137 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; answer_run "ls $(echo $HOME)/House/Room/" justumen "Non"; restore=$(expr $restore + 1) ;&
...
140) echo -n 140 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Donc ici le dernier '/' dans ${learn}$(echo $HOME)/House/Room/${reset} n'est pas obligatoire. Testez donc a nouveau avec ${learn}ls $(echo $HOME)/House/Room${reset}"; restore=$(expr $restore + 1) ;&
141) echo -n 141 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; answer_run "ls $(echo $HOME)/House/Room" justumen "Non"; restore=$(expr $restore + 1) ;&
142) echo -n 142 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Pas de problème, le résultat est le même pour ${learn}ls $(echo $HOME)/House/Room/${reset} et ${learn}ls $(echo $HOME)/House/Room${reset}."; restore=$(expr $restore + 1) ;&
143) echo -n 143 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Quand vous avez fait ${learn}rm virus${reset} pour la première suppression, vous avez utilisé ce que l'on appelle le ${voc}chemin relatif${reset}."; restore=$(expr $restore + 1) ;&
...
146) echo -n 146 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Si ${learn}pwd${reset} vous donne ${learn}$(echo $HOME)${reset}. Un ${learn}rm virus${reset} ne supprimera aucun d'entre eux. Cette commande voudra supprimer le fichier ${learn}/home/umen/virus${reset}."; restore=$(expr $restore + 1) ;&
147) echo -n 147 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; cd "$(echo $HOME)"; restore=$(expr $restore + 1) ;&
...
151) echo -n 151 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Pour changer de ${voc}répertoire courant${reset}, vous pouvez utiliser la commande ${learn}cd${reset}. (${learn}C${reset}hange ${learn}D${reset}irectory)"; restore=$(expr $restore + 1) ;&
152) echo -n 152 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Pour revenir dans le répertoire 'Room', vous pouvez utiliser le ${voc}chemin absolu${reset} avec la commande ${learn}cd $(echo $HOME)/House/Room/${reset}"; restore=$(expr $restore + 1) ;&
153) echo -n 153 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Mais il n'est pas obligatoire d'utiliser le ${voc}chemin absolu${reset}, il est possible de revenir dans le répertoire 'Room' en utilisant un ${voc}chemin relatif${reset}."; restore=$(expr $restore + 1) ;&
154) echo -n 154 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Vous voulez aller dans ${learn}$(echo $HOME)/House/Room/${reset} mais vous êtes déjà dans ${learn}$(echo $HOME)${reset}."; restore=$(expr $restore + 1) ;&
...
165) echo -n 165 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Il commence toujours à la racine, c'est à dire que le premier caractère d'un ${voc}chemin absolu${reset} est toujours '/'."; restore=$(expr $restore + 1) ;&
166) echo -n 166 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Il y a aussi une syntaxe spéciale très utile pour le ${voc}chemin relatif${reset} les ${learn}..${reset}"; restore=$(expr $restore + 1) ;&
167) echo -n 167 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "${learn}..${reset} représente dans l'arborescence linux le parent du ${voc}répertoire courant${reset}."; restore=$(expr $restore + 1) ;&
...
171) echo -n 171 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Cibler les enfants en ${voc}argument${reset} avec un ${voc}chemin relatif${reset} est très simple, il suffit d'ecrire le nom des parents successifs."; restore=$(expr $restore + 1) ;&
...
174) echo -n 174 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Affichez le chemin absolu de votre ${voc}répertoire courant${reset}."; restore=$(expr $restore + 1) ;&
175) echo -n 175 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Vous connaissez maintenant la commande pour le changer : ${learn}cd${reset}."; restore=$(expr $restore + 1) ;&
176) echo -n 176 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Ici nous allons nous déplacer dans le répertoire parent. Nous sommes dans ${learn}$(echo $HOME)/House/Room/${reset} mais nous voulons aller dans ${learn}$(echo $HOME)/House/${reset}"; restore=$(expr $restore + 1) ;&
...
182) echo -n 182 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Mais il nous reste deux virus à supprimer, commençons par supprimer le fichier virus3 avec le ${voc}chemin relatif${reset}."; restore=$(expr $restore + 1) ;&
...
184) echo -n 184 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk_not_press_key justumen "Bien. Maintenant supprimons le fichier virus4 en utilisant le ${voc}chemin absolu${reset}."; restore=$(expr $restore + 1) ;&
185) echo -n 185 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; answer_run "rm $(echo $HOME)/House/Room/virus4" justumen "Non"; restore=$(expr $restore + 1) ;&
186) echo -n 186 > $HOME/.GameScript/restore_bash1; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash1; talk justumen "Parfait ! Felicitations, vous avez tout compris."; restore=$(expr $restore + 1) ;&
