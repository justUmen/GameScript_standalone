...
63) { echo -n 63 > $HOME/.GameScript/restore_bash10 } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash10 } 2>/dev/null ; talk justumen "${code}\$?${reset} contient un nombre entre 0 et 255, qui est la ${voc}valeur de retour${reset} de votre dernière commande. (anglais ${voc}exit status${reset})"; restore=$(expr $restore + 1) ;&
...
66) { echo -n 66 > $HOME/.GameScript/restore_bash10 } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash10 } 2>/dev/null ; talk justumen "Ici la valeur retour est 0. Cela veut dire que la commande ${code}pwd${reset} n'a pas rencontré de problème."; restore=$(expr $restore + 1) ;&
...
69) { echo -n 69 > $HOME/.GameScript/restore_bash10 } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash10 } 2>/dev/null ; talk justumen "Ici la valeur retour est 127. Cela veut dire que la commande ${codeError}pwdd${reset} n'existe pas."; restore=$(expr $restore + 1) ;&
...
72) { echo -n 72 > $HOME/.GameScript/restore_bash10 } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash10 } 2>/dev/null ; talk justumen "Ici la valeur retour est encore 127. Cela veut dire que la commande ${codeError}lss${reset} n'existe pas."; restore=$(expr $restore + 1) ;&
...
75) { echo -n 75 > $HOME/.GameScript/restore_bash10 } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash10 } 2>/dev/null ; talk justumen "Ici la valeur retour n'est pas 0, ce qui veut dire que la commande a rencontré une erreur."; restore=$(expr $restore + 1) ;&
...
83) { echo -n 83 > $HOME/.GameScript/restore_bash10 } 2>/dev/null ; { echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash10 } 2>/dev/null ; talk_not_press_key justumen "Essayez donc de sauvegarder la valeur retour de ${code}ls -O${reset} avec : ${learn}ls -O;VAL=\$?${reset}"; restore=$(expr $restore + 1) ;&
