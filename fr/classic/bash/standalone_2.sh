...
talk justumen "Cet argument peut avoir un nom spécial, comme ${codeFile}..${reset} qui représente le répertoire parent."
talk justumen "${codeFile}..${reset} étant l'abréviation d'un répertoire, il peut également s'écrire ${codeFile}../${reset}"
talk justumen "${codeFile}..${reset} étant lui même un répertoire, il peut avoir un parent, qui peut être ciblé en argument par ${codeFile}../..${reset} ou encore ${codeFile}../../${reset}."
talk justumen "${codeFile}../..${reset} cible donc le grand parent du répertoire courant, ${codeFile}../../..${reset} l'arrière grand parent et ainsi de suite."
talk justumen "Mais il existe un autre nom spécial, qui représente le répertoire courant : ${codeFile}.${reset}."
talk justumen "Comme il s'agit d'un dossier, ${codeFile}./${reset} est également correct."
...
answer_run "ls ./" justumen "Non"
talk justumen "Oui toutes ces ${learn}ls${reset} donnent le même résultat. Sans argument, la commande cible par défaut le répertoire ${codeFile}.${reset}."
talk justumen "Vous l'avez compris, ${codeFile}.${reset} est l'abréviation du répertoire courant !"
talk_not_press_key justumen "Pour supprimer le fichier ${codeFile}bOb${reset}, vous pouvez donc taper ${learn}rm ./bOb${reset}, allez-y."
...
talk justumen "Attention à ce symbole ${codeFile}.${reset}, dans une autre situation il peut avoir une autre signification."
talk justumen "Ici ce ${codeFile}.${reset} dans ${learn}$HOME/.GameScript_bash2${reset} fait simplement partie du nom du dossier."
talk justumen "Le nom du dossier est ${codeFile}.GameScript_bash2${reset}."
talk justumen "Mais sur Linux si un nom de fichier ou dossier commence par un ${codeFile}.${reset}, il sera ${voc}caché${reset}."
talk justumen "En faisant ${learn}pwd${reset}, vous avez bien vu que le terminal est dans le dossier ${codeFile}${HOME}/.GameScript_bash2${reset}."
talk justumen "Ce dossier est donc bien évidemment dans ${codeFile}${HOME}${reset} !"
talk justumen "Mais si vous tentez d'afficher les éléments de ${codeFile}${HOME}${reset}, vous ne le ve verrez pas."
...
talk justumen "Vous êtes maintenant dans ${codeFile}${HOME}${reset}."
...
talk justumen "Ces arguments spéciaux commence par un ${codeFile}-${reset} et se nomment ${codeFile}options${reset}."
...
talk justumen "Ici vous devriez voir le dossier ${codeFile}.GameScript_bash2${reset}, mais aussi beaucoup d'autres éléments ${voc}cachés${reset}."
talk justumen "Maintenant comment revenir dans ce dossier ${codeFile}.GameScript_bash2${reset} ?"
talk justumen "Vous l'avez compris ${codeFile}./${reset} représente le répertoire courant."
...
talk justumen "J'espère que la différence entre ces deux ${codeFile}.${reset} dans cette commande est pour vous évidente."
talk_not_press_key justumen "Maintenant affichez les fichiers ${voc}cachés${reset} de ce répertoire."
answer_run "ls -a" justumen "Non"
talk justumen "Il n'y en a aucun, mais vous pouvez en revanche voir ${codeFile}.${reset} et ${codeFile}..${reset}"
talk justumen "Maintenant que l'on parle de répertoire avec symbole spéciaux, nous pouvons parler de ${codeFile}~${reset}."
talk justumen "${codeFile}~${reset} représente le répertoire de base de l'utilisateur."
talk justumen "Le plus souvent ce répertoire porte le nom de l'utilisateur dans ${codeFile}/home/${reset}."
talk justumen "Dans votre cas, ${codeFile}~${reset} s'agit de ${codeFile}$HOME${reset}."
talk justumen "Ce ${codeFile}~${reset} étant un répertoire, vous pouvez utilisez ce symbole comme un répertoire."
...
answer_run "pwd" justumen "Non"
talk justumen "${codeFile}~${reset} remplace dans votre cas ${codeFile}$HOME${reset}"
talk justumen "Pour cibler ce dossier, vous pouvez donc utiliser comme chemin relatif ${codeFile}.GameScript_bash2${reset} ou ${codeFile}./.GameScript_bash2${reset}"
talk justumen "Mais vous avez également, grace à ${codeFile}~${reset}, utiliser un nouveau chemin absolu : ${codeFile}~/.GameScript_bash2${reset}"
...
answer_run "cd /" justumen "Non"
talk_not_press_key justumen "Maintenant supprimez le dossier ${codeFile}.GameScript_bash2${reset} en utilisant le symbole ${codeFile}~${reset} dans votre commande !"
answer_run "rmdir ~/.GameScript_bash2" justumen "Non"
talk justumen "Parfait !"
talk justumen "Maintenant revenons sur les commandes et leurs ${codeFile}options${reset}."
talk_not_press_key justumen "Listez les fichiers ${voc}cachés${reset} du répertoire courant avec ${learn}ls -a${reset}."
answer_run "ls -a" justumen "Non"
talk justumen "La plupart des arguments ont aussi une version longue, parfois plus facile à mémoriser, qui commence elle par ${codeFile}--${reset}."
talk justumen "Par exemple vous pouvez remplacer ${codeFile}-a${reset} par ${codeFile}--all${reset}."
talk justumen "Les commandes ${codeFile}ls -a${reset} et ${codeFile}ls --all${reset} sont identiques !"
talk_not_press_key justumen "Listez les fichiers ${voc}cachés${reset} de ${codeFile}$HOME${reset}  ${learn}ls --all $HOME${reset}."
answer_run "ls --all $HOME" justumen "Non"
talk justumen "Mais comment pouvoir retenir autant d'options ?"
talk justumen "En fait vous n'avez pas besoin de les mémoriser car vous pouvez toujours accéder au ${codeFile}manuel${reset} d'une commande avec la commande ${learn}man${reset}."
...
talk justumen "La plupart des commandes ont une option ${codeFile}--help${reset}, qui affiche l'aide de la commande, parfois simplement le contenu du manuel."
...
talk justumen "Mais il est possible de limiter sa longueur avec l'option ${codeFile}-w${reset}."
talk_not_press_key justumen "Ouvrez le manuel de ${codeFile}ls${reset}, lisez les détails de l'option ${codeFile}-w${reset} et quittez le manuel."
answer_run "man ls" justumen "Non"
talk justumen "Certaines options, comme ici ${codeFile}-w${reset}, peuvent aussi avoir des valeurs."
talk justumen "Ici il faudra donner une valeur numérique à ${codeFile}-w${reset}."
...
talk justumen "La version longue des options avec valeur est parfois différente avec l'utilisation du signe ${codeFile}=${reset} au lieu d'un espace."
