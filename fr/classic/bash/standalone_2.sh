...
talk justumen "Cet argument peut avoir un nom spécial, comme ${voc}..${reset} qui représente le répertoire parent."
talk justumen "${voc}..${reset} étant l'abréviation d'un répertoire, il peut également s'écrire ${voc}../${reset}"
talk justumen "${voc}..${reset} étant lui même un répertoire, il peut avoir un parent, qui peut être ciblé en argument par ${voc}../..${reset} ou encore ${voc}../../${reset}."
talk justumen "${voc}../..${reset} cible donc le grand parent du répertoire courant, ${voc}../../..${reset} l'arrière grand parent et ainsi de suite."
talk justumen "Mais il existe un autre nom spécial, qui représente le répertoire courant. Il s'agit du ${voc}.${reset} ou bien évidemment ${voc}./${reset}."
...
answer_run "ls ./" justumen "Non"
talk justumen "Oui toutes ces commandes donnent le même résultat, sans argument la commande cible par défaut le répertoire ${voc}.${reset}."
talk justumen "Vous l'avez compris, ${voc}.${reset} est l'abréviation du répertoire courant !"
talk justumen "Mais attention à ce symbole ${voc}.${reset}, dans une autre situation il peut avoir une autre signification."
cd $HOME/.GameScript_bash2
talk justumen "Je viens de vous déplacer dans un nouveau dossier."
talk_not_press_key justumen "Affichez donc son chemin absolu."
answer_run "pwd" justumen "Non"
talk justumen "Ici ce ${voc}.${reset} dans ${learn}$HOME/.GameScript_bash2${reset} fait just partie du nom du dossier."
talk justumen "Le nom du dossier est simplement ${voc}.GameScript_bash2${reset}."
talk justumen "Mais sur Linux si un nom de fichier ou dossier commence par un ${voc}.${reset}, il sera ${voc}caché${reset}"
talk justumen "En faisant ${learn}pwd${reset}, vous avez bien vu que le terminal est dans le dossier ${voc}${HOME}/.GameScript_bash2${reset}."
talk justumen "Ce dossier est donc bien évidemment dans ${voc}${HOME}${reset} !"
talk justumen "Mais si vous tentez d'afficher les éléments de ${voc}${HOME}${reset}, vous ne le ve verrez pas."
...
talk justumen "Vous êtes maintenant dans ${voc}${HOME}${reset}."
...
talk justumen "Ces arguments spéciaux commence par un ${voc}-${reset} et se nomme ${voc}options${reset}."
talk_not_press_key justumen "Pour que ${learn}ls${reset} affiche les fichiers cachés, tapez donc ${learn}ls -a${reset}"
answer_run "ls -a" justumen "Non"
talk justumen "Ici vous devriez voir le dossier ${voc}.GameScript_bash2${reset}, mais aussi beaucoup d'autres éléments cachés."
talk justumen "Maintenant comment revenir dans ce dossier ${voc}.GameScript_bash2${reset} ?"
talk justumen "Vous l'avez compris ${voc}./${reset} représente le répertoire courant."
talk_not_press_key justumen "C'est à dire qu'il est possible de se déplacer dans ce répertoire avec : ${learn}cd ./.GameScript_bash2${reset}, essayez donc cette commande."
answer_run "cd ./.GameScript_bash2" justumen "Non"
talk justumen "J'espère que la différence entre ces deux ${voc}.${reset} dans cette commande sont pour vous évidente."
talk_not_press_key justumen "Maintenant affichez les fichiers cachés de ce répertoire."
answer_run "ls -a" justumen "Non"
talk justumen "Il n'y en a aucun, mais vous pouvez en revanche voir ${voc}.${reset} et ${voc}..${reset}"
talk justumen "Maintenant que l'on parle de répertoire avec symbole spéciaux, nous pouvons parler de ${voc}~${reset}."
talk justumen "${voc}~${reset} représente le répertoire de base de l'utilisateur."
talk justumen "Le plus souvent ce répertoire porte le nom de l'utilisateur dans ${voc}/home/${reset}."
talk justumen "Dans votre cas, ${voc}~${reset} s'agit de ${voc}$HOME${reset}."
talk justumen "Ce ${voc}~${reset} étant un répertoire, vous pouvez utilisez ce symbole comme un répertoire."
...
answer_run "pwd" justumen "Non"
talk justumen "${voc}~${reset} remplace dans votre cas ${voc}$HOME${reset}"
talk justumen "Pour cibler ce dossier, vous pouvez donc utiliser comme chemin relatif ${voc}.GameScript_bash2${reset} ou ${voc}./.GameScript_bash2${reset}"
talk justumen "Mais vous avez également, grace à ${voc}~${reset}, un nouveau chemin absolu : ${voc}~/.GameScript_bash2${reset}"
talk_not_press_key justumen "Déplacez vous donc dans le répertoire racine."
answer_run "cd /" justumen "Non"
talk_not_press_key justumen "Maintenant supprimez le dossier ${voc}.GameScript_bash2${reset} en utilisant le symbole ${voc}~${reset} dans votre commande !"
answer_run "rmdir ~/.GameScript_bash2" justumen "Non"
talk justumen "Parfait !"
talk justumen "Maintenant revenons sur les commandes et leurs ${voc}options${reset}."
talk_not_press_key justumen "Listez les fichiers cachés du répertoire courant avec ${learn}ls -a${reset}."
answer_run "ls -a" justumen "Non"
talk justumen "La plupart des arguments ont aussi une version longue, parfois plus facile à mémoriser, qui commence elle par ${voc}--a${reset}."
talk justumen "Par exemple vous pouvez remplacer ${voc}-a${reset} par ${voc}--all${reset}."
talk_not_press_key justumen "Listez les fichiers cachés du répertoire courant avec ${learn}ls --all${reset}."
answer_run "ls --all" justumen "Non"
talk justumen "Mais comment pouvoir retenir autant d'options ?"
talk justumen "En fait vous n'avez pas besoin de les mémoriser car vous pouvez toujours accéder au ${voc}manuel${reset} d'une commande avec la commande ${learn}man${reset}."
...
talk justumen "Apprendre à utiliser les manuels est indispensable pour pourvoir se débrouiller seul."
...
talk justumen "Si vous ne vous souvenez plus de ce que je fais une commande, il vous suffit de visiter son manuel."
...
talk justumen "La plupart des commandes ont une option ${voc}--help${reset}, qui affiche l'aide de la commande, parfois simplement le contenu du manuel."
talk justumen "L'option courte de ${voc}--help${reset} est ${voc}-h${reset}."
talk_not_press_key justumen "Affichez l'aide de rmdir avec ${learn}rmdir -h${reset}"
answer_run "rmdir -h" justumen "Non"
...
talk justumen "Mais il est possible de limiter sa longueur avec l'option ${voc}-w${reset}."
talk_not_press_key justumen "Ouvrez le manuel de ${voc}ls${reset}, lisez les détails de l'option ${voc}-w${reset} et quittez le manuel."
answer_run "man ls" justumen "Non"
talk justumen "Certaines options, comme ici ${voc}-w${reset}, peuvent aussi avoir des valeurs."
talk justumen "Ici il faudra donner une valeur numérique à ${voc}-w${reset}."
...
talk justumen "La version longue des options avec valeur est souvent différente avec l'utilisation du signe ${voc}=${reset} au lieu d'un espace."
...
talk justumen ""
talk justumen ""~""
talk justumen "".hidden""
talk justumen ""../""
talk justumen ""../../""
talk justumen ""." (useful to execute)"
talk justumen "command line argument(s) "ls ../../""
talk justumen "command line options (dash) "ls -l /" or "ls / -l""
talk justumen "options often have long and short names"
talk justumen "sometimes options have values "ls --sort=size""
talk justumen ""man" RTFM (you shouldnt ask someone about something that is in the manual)"
talk justumen "if you want to do something, check the manual first,"
talk justumen "if you cant find your answer then you can ask for help"
talk justumen ""
talk justumen "èéà"
}
function quiz(){
