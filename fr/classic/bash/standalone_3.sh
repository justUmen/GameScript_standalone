...
2) echo -n 2 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; mkdir $HOME/.GameScript_bash3 && mkdir -p $HOME/.GameScript/Audio/fr/bash/c3/ && wget -q -nc $AUDIO_DL/4.mp3 -O $HOME/.GameScript/Audio/fr/bash/c3/4.mp3 > /dev/null 2>&1; restore=$(expr $restore + 1) ;&
...
120) echo -n 120 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Essayez par exemple d'afficher dans votr terminal : ${code}\\quitter${reset}."; restore=$(expr $restore + 1) ;&
...
133) echo -n 133 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Si vous avez de nombreux ${code}'${reset} à afficher, je vous conseille d'utiliser les ${code}\"${reset} comme délimiteur, et si vous avez de nombreux ${code}\"${reset} à afficher, je vous conseille d'utiliser les ${code}\'${reset} comme délimiteur."; restore=$(expr $restore + 1) ;&
