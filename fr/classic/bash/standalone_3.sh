...
2) echo -n 2 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; mkdir $HOME/.GameScript_bash3; restore=$(expr $restore + 1) ;&
...
5) echo -n 5 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Si vous avez plusieures options à passer à la même commande, vous pouvez les mettre les unes après les autres : ${learn}ls -a -w 1${reset}."; restore=$(expr $restore + 1) ;&
6) echo -n 6 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Essayez donc avec l'exemple que l'on a déjà vu : ${learn}ls -a -w 1${reset}"; restore=$(expr $restore + 1) ;&
...
8) echo -n 8 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Vous pouvez bien évidemment aussi utiliser les versions longues de la même manière."; restore=$(expr $restore + 1) ;&
9) echo -n 9 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Essayez donc la même chose avec : ${learn}ls --all --width=1${reset}"; restore=$(expr $restore + 1) ;&
10) echo -n 10 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; answer_run "ls --all --width=1" justumen "Non"; restore=$(expr $restore + 1) ;&
...
12) echo -n 12 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Si vous utilisez les versions courtes, vous pouvez également les regrouper avec le même ${code}-${reset}."; restore=$(expr $restore + 1) ;&
...
18) echo -n 18 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "${learn}ls -a${reset} est identique à ${learn}ls --all${reset}."; restore=$(expr $restore + 1) ;&
...
25) echo -n 25 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Le script que vous utilisez en ce moment affiche du texte dans votre terminal."; restore=$(expr $restore + 1) ;&
26) echo -n 26 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Ce script étant lui-même en bash, quelle commande est utilisée ici pour affichez ces phrases ?"; restore=$(expr $restore + 1) ;&
...
28) echo -n 28 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Il vous renverra simplement un écho de ce que vous lui donnez en argument."; restore=$(expr $restore + 1) ;&
...
34) echo -n 34 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "On a déjà vu ensemble que ${code}mkdir${reset} est utilisé pour créer de nouveaux dossiers."; restore=$(expr $restore + 1) ;&
...
42) echo -n 42 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Lorsque vous utilisez ${code}>${reset}, si le fichier n'existe pas il sera créé."; restore=$(expr $restore + 1) ;&
...
46) echo -n 46 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Ce contenu est le résultat de la commande à gauche du symbole ${code}>${reset}."; restore=$(expr $restore + 1) ;&
...
51) echo -n 51 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "La commande ${code}cat${reset} porte ce nom car elle peut être utilisé pour la con${code}cat${reset}énation. (Mettre bout à bout des chaines de caractère.)"; restore=$(expr $restore + 1) ;&
...
56) echo -n 56 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "${learn}cat test test${reset} affiche ici deux fois le contenu du fichier 'test'."; restore=$(expr $restore + 1) ;&
...
58) echo -n 58 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Si le fichier existe déjà le contenu sera remplacé."; restore=$(expr $restore + 1) ;&
...
61) echo -n 61 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Affichez maintenant le fichier 'test'."; restore=$(expr $restore + 1) ;&
...
63) echo -n 63 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Ici le contenu a été remplacé ! Les mots 'bonjour' n'existent plus."; restore=$(expr $restore + 1) ;&
...
65) echo -n 65 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Si vous voulez ${voc}concaténer${reset} du nouveau contenu dans ce fichier il vous faudra utiliser ${code}>>${reset}."; restore=$(expr $restore + 1) ;&
66) echo -n 66 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Encore une fois si vous utilisez ${code}>>${reset}, si le fichier n'existe pas il sera créé."; restore=$(expr $restore + 1) ;&
...
70) echo -n 70 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Puis affichez le fichier 'test'."; restore=$(expr $restore + 1) ;&
...
73) echo -n 73 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Vous pouvez continuer à utiliser ${code}>>${reset} pour ajouter le contenu les uns à la suite des autres."; restore=$(expr $restore + 1) ;&
74) echo -n 74 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Mais ces redirections (${code}>>${reset} et ${code}>${reset}) ne se limitent pas à ${code}echo${reset}."; restore=$(expr $restore + 1) ;&
...
76) echo -n 76 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Mettez par exemple le résultat de la commande ${learn}ls${reset} à la suite de ce  !"; restore=$(expr $restore + 1) ;&
...
80) echo -n 80 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "C'est le ${voc}résultat${reset} de la commande qui sera redirigé. Ici ${learn}ls>>test${reset} donne comme résultat : 'test'."; restore=$(expr $restore + 1) ;&
...
83) echo -n 83 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Ajoutons les trois lettres : 'pwd' à la fin du fichier 'test'."; restore=$(expr $restore + 1) ;&
...
99) echo -n 99 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Mais comment afficher cet espace pour que la suite ne soit pas considéré comme un nouvel argument ?"; restore=$(expr $restore + 1) ;&
...
101) echo -n 101 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Sur bash, il s'agit du caractère ${code} \ ${reset}."; restore=$(expr $restore + 1) ;&
102) echo -n 102 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Le caractère d'échappement affectera uniquement le prochain caractère."; restore=$(expr $restore + 1) ;&
103) echo -n 103 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Dans notre cas pour représenter un espace nous pouvons utiliser ${code}\\ ${reset}."; restore=$(expr $restore + 1) ;&
...
106) echo -n 106 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Cependant dans cet exemple il y a toujours deux arguments, le premier est 'a' et le deuxième est '\\ b'."; restore=$(expr $restore + 1) ;&
...
113) echo -n 113 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Comme par exemple le symbole ${learn}>${reset}, qui comme vous le savez est aussi interprété par bash comme un caractère spécial."; restore=$(expr $restore + 1) ;&
...
117) echo -n 117 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "${learn}echo x>y${reset} va créer un nouveau fichier 'y' avec comme contenu 'x' !"; restore=$(expr $restore + 1) ;&
...
119) echo -n 119 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Pour affichez le symbole ${code} \ ${reset} avec echo, il suffit de rajouter avant votre caractère d'échappement."; restore=$(expr $restore + 1) ;&
120) echo -n 120 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Essayez par exemple d'afficher : ${code}\\quitter${reset}."; restore=$(expr $restore + 1) ;&
...
122) echo -n 122 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Dans ${code} \\\\\\quitter ${reset}, le premier ${code} \ ${reset} est le caractère d'échappement, mais le deuxième est simplement le caractère qui doit être interprété littéralement par le terminal."; restore=$(expr $restore + 1) ;&
123) echo -n 123 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Pour ne pas avoir à utiliser ce ${code} \ ${reset} à chaque espace, vous pouvez également utiliser les ${code}\"${reset}."; restore=$(expr $restore + 1) ;&
...
131) echo -n 131 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Vous pouvez aussi remplacer les ${code}\"${reset} par des ${code}'${reset} : ${learn}echo 'X   X'${reset}"; restore=$(expr $restore + 1) ;&
...
133) echo -n 133 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Si vous avez de nombreux ${code}'${reset} à afficher, utilisez ${code}\"${reset} comme délimiteur, et vice versa."; restore=$(expr $restore + 1) ;&
134) echo -n 134 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Par exemple, ${learn}echo '\"Pierre\" et \"Marie\"'${reset} est une commande plus lisible que ${learn}echo \"\\\"Pierre\\\" et \\\"Marie\\\"\"${reset}."; restore=$(expr $restore + 1) ;&
...
139) echo -n 139 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk justumen "Sur bash, la lettre 'n' est utilisée avec ${code} \ ${reset} pour représenter une mise à la ligne, 'n' comme ${code}n${reset}ouvelle ligne."; restore=$(expr $restore + 1) ;&
140) echo -n 140 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Affichez donc la lettre 'a', puis la lettre 'b' sur une nouvelle ligne en utilisant ${code}'${reset} comme délimiteur."; restore=$(expr $restore + 1) ;&
...
142) echo -n 142 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Trouvez la syntaxe d'une ${voc}tabulation${reset} horizontale dans le manuel de la commande echo."; restore=$(expr $restore + 1) ;&
...
144) echo -n 144 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "En utilisant ${code}'${reset}, affichez la lettre 'a', suivi d'une tabulation, puis de la lettre 'b'."; restore=$(expr $restore + 1) ;&
...
150) echo -n 150 > $HOME/.GameScript/restore_bash3; echo -n $(pwd) > $HOME/.GameScript/restore_pwd_bash3; talk_not_press_key justumen "Affichez maintenant le fichier 'test' puis le fichier 'tab' à la suite."; restore=$(expr $restore + 1) ;&
